...
export const ForecastingChartThree = (props: AnomaliesLiveChartProps) => {
  ...
  const [xAxisMax, setxAxisMax] = useState(5);
  ...
  const [brushStart, setBrushStart] = useState(1);
  const [brushEnd, setBrushEnd] = useState(3);
  //const [xDomain, setXDomain] = useState({ min: 0, max: 20 });
  ...
  //     const { x } = event;
  //     setXDomain({
  //       min: Math.max(0, Math.floor(x[0])),
  //       max: Math.min(20, Math.ceil(x[2])),
  //     });
  //   }
  // };
  const [brushExtent, setBrushExtent] = useState(undefined);
  ...
  const [valueSlider, setValueSlider] = useState(['0', '20']);
  const [value2, setValue2] = useState(['0', '5']);

  const setSliderHandler = (values) => {
    setxAxisMin(values[0]);
    setxAxisMax(values[1]);
    setValue2(values)
  }; 

  const [xDomain, setXDomain] = useState([dataTest[0].x, dataTest[dataTest.length - 1].x]);

  const handleBrushEnd = (brushArea) => {
    if (brushArea.x) {
      setXDomain([brushArea.x[0], brushArea.x[1]]);
    }
  };
    ...
    const handleDragStart = (event) => {
      console.log("event: " + JSON.stringify(event))
      const { x0, y0, x1, y1 } = rectPosition[0].coordinates;
      const dx = event.x - x0;
      const dy = event.y - y0;
      setRectPosition([
        {
              ...
              >
                ...
                {/* <Chart>
                  ...
                  {/* <LineSeries
                    ...
                    //enableBrush={{ fixedSize: 2, resetOnEnd: false }}
                  /> */}
      {/* <LineSeries
          id="line"
          xScaleType={ScaleType.Time}
          ...
          data={dataTest}
        /> */}
        <RectAnnotation onDrag={handleDragStart} onDragEnd={handleRectDragEnd} style={{ fill: 'red' }} dataValues={rectPosition} id="red1"/>
          
     {/* {brushExtent && (
            <RectAnnotation
              x={brushExtent[0]}
              y={0}
              width={brushExtent[1] - brushExtent[0]}
              height={100}
              fill="gray"
              opacity={0.2}
            />
