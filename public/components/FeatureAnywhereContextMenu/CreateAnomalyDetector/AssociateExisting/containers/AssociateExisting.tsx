...
interface AssociateExistingProps {
  ...
  setSelectedDetectorId(detectorID: string): void;
}
...
export function AssociateExisting(
  ...
  useEffect(() => {
    ...
    savedObjectLoader.findAll().then((resp: any) => {
      if (resp != undefined) {
        ...
        console.log(
          'currentAssociatedDetectors: ' + curDetectorsToDisplayOnList
        );
  ...
  const getExistingDetectorsAvalableToAssociate = (
    ...
    console.log('vizID: ' + associateExistingProps.embeddableVisId);
  ...
  const selectedOptions = useMemo(() => {
    if (
      !existingDetectorsAvailableToAssociate ||
      !associateExistingProps.selectedDetectorId
    ) {
      return [];
    }

    const detector = (existingDetectorsAvailableToAssociate || []).find(
      (detector) => detector.id === associateExistingProps.selectedDetectorId
    );
  ...
  return (
    <div className="associate-existing">
      <EuiText size="xs">
        <p>
          Hello This is a short description of the feature to get users excited.
      ...
      {existingDetectorsAvailableToAssociate && (
        <EuiComboBox
          ...
          onChange={(selectedOptions) => {
            ...
            associateExistingProps.setSelectedDetectorId(id);
          }}
